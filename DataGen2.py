...
class DirectoryIterator(Iterator):
    ...
    def __init__(self, directory, pos_labels_dir, neg_labels_dir, image_data_generator, output_type="h",
                 ...
                 pos_batch_size=14, neg_batch_size=50, shuffle=True, seed=None,
                 data_format=None,
    ...
    def next(self):
        ...
        for i, j in enumerate(index_array):
            if i<self.pos_batch_size:
                x, l_dict = self.get_img(j, self.pos_labels)
            else:
                x, l_dict = self.get_img(j, self.neg_labels)
            batch_x.append(x)
            batch_y_fnf.append(l_dict["labelFnf"])
            batch_y_landmarks.append(l_dict["labelLandmarks"])
